...
int function::find_loop_length(const std::vector<double>& wave, uint32_t sample_rate, uint16_t channels) {
    ...
    int template_len = sample_rate * channels * 4;
    ...
    int start_idx = N_total - template_len;

    while (start_idx >= 0) {
        start_idx = (start_idx / channels) * channels;
        ...
        for (int i = 0; i < check_window; ++i) {
            start_vol_sum += std::abs(wave[start_idx + i]);
            end_vol_sum += std::abs(wave[start_idx + template_len - check_window + i]);
        }

        double start_avg = start_vol_sum / check_window;
        double end_avg = end_vol_sum / check_window;

        if (start_avg > 0.0001 && end_avg >= start_avg * 0.25) {
            break;
        }

        start_idx -= check_window / 2;
    }

    double sec_from_end = static_cast<double>(N_total - template_len - start_idx) / (sample_rate * channels * 1);
    std::cout << "末尾から" << sec_from_end << "秒前の波形を採用" << std::endl;
    ...
    for (int i = 0; i < fft_size; ++i) {
        cross_corr[i] = wave_c[i] * std::conj(temp_c[i]);
    }
    ifft(cross_corr);

    int peak1 = start_idx;
    int peak2 = -1;
    double max_raw_score = -1.0;
    ...
    for (int i = 0; i < N_total - template_len; ++i) {
        ...
        double raw_score = cross_corr[i].real();
        if (raw_score > max_raw_score * 0.99) {
            peak2 = i;
            break;
        }
    }

    int best_loop_length = std::abs(peak2 - peak1);
    ...
    for (int t = 0; t < num_tests; ++t) {
        ...
        for (int i = 0; i < template_len; ++i) {
            double val = wave[start_idx + i];
            temp_c[i] = val; 
            template_energy += val * val;
        }
        ...
        for (int i = 0; i < N_total - template_len; ++i) {
            ...
            if (raw_score > current_max_score) {
                ...
                peak2 = i;
            } else if (current_max_score > 0 && raw_score > current_max_score * 0.95) {
                if (std::abs(i - peak1) > std::abs(peak2 - peak1)) {
                    peak2 = i;
                }
            }
        }
...
std::vector<int> function::find_loop_start(const std::vector<double>& wave, int loop_length, uint32_t sample_rate, uint16_t channels) {
    ...
    int match_required = (sample_rate * channels) * 4;
